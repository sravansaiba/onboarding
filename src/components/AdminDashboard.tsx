...
//         { id: 2, name: 'Business', icon: 'document' },
//         { id: 3, name: 'Operations', icon: 'clock' },
...
//                 {/* Step Navigation */}
//                 <div className="bg-gray-50 px-3 sm:px-6 py-3 sm:py-4 overflow-x-auto">
//                     <div className="flex justify-between max-w-3xl mx-auto min-w-max">
//                         {steps.map((step, idx) => (
//                             <div key={step.id} className="flex flex-col items-center min-w-[80px]">
//                                 <button
//                                     onClick={() => setCurrentStep(step.id)}
//                                     className={`flex flex-col items-center gap-1 px-2 py-1.5 sm:px-3 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition ${currentStep === step.id
//                                             ? 'text-orange-600 bg-orange-50'
//                                             : 'text-gray-500 hover:bg-gray-100'
//                                         }`}
//                                 >
//                                     <span className={`${currentStep === step.id ? 'text-orange-600' : 'text-gray-500'}`}>
//                                         {stepIcons[step.icon]}
//                                     </span>
//                                     <span>{step.name}</span>
//                                 </button>
//                                 {idx < steps.length - 1 && (
//                                     <div className="w-6 sm:w-8 h-0.5 bg-gray-200 mx-1 mt-1 hidden sm:block"></div>
//                                 )}
//                             </div>
//                         ))}
//                     </div>
...
import { ChevronLeftCircle, ChevronLeftCircleIcon } from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    return (
        <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100">
            ...
            {/* Modern Header */}
            <div className="bg-white border-b border-gray-200 sticky top-0  z-40">
                <div className="flex justify-between  px-4 sm:px-6 lg:px-8 py-4">     
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">  
                        
                        <div>
                            <button
                                onClick={() => router.push('/')}
                                className="px-7 py-2 text-sm text-gray-700  rounded-3xl hover:-translate-x-0.5  transition"
                            >
                                <ChevronLeftCircleIcon className="w-6 h-6" />
                            </button>
                        </div>
                        <div> 
                            <h1 className="text-2xl sm:text-3xl font-bold bg-linear-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">
                            Welcome {username} ! 
                            </h1>
                            <p className="text-gray-600 mt-1">Restaurant Onboarding Management</p>
                        </div>
                    ...
                    <div>
                        <button
                            ...
                            className="px-4 py-2.5 sm:px-5 sm:py-2.5 bg-linear-to-r from-red-500 to-red-600 text-white text-sm sm:text-base rounded-lg hover:shadow-lg transition">
                            Logout
                        </button>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
                    <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm p-4 sm:p-6 border border-gray-100 hover:shadow-md transition">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs sm:text-sm font-medium text-gray-600">Total Applications</p>
                                <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-1 sm:mt-2">{records.length}</p>
                            </div>
                            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-blue-100 to-blue-50 rounded-xl flex items-center justify-center">
                    ...
                    <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm p-4 sm:p-6 border border-gray-100 hover:shadow-md transition">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-green-100 to-green-50 rounded-xl flex items-center justify-center">
                    ...
                    <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm p-4 sm:p-6 border border-gray-100 hover:shadow-md transition">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-orange-100 to-orange-50 rounded-xl flex items-center justify-center">
                ...
                <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Restaurant</th>
                                    <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Owner</th>
                                    <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Package</th>
                                    <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Status</th>
                                    <th className="px-4 sm:px-6 py-3 sm:py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Action</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100">
                                {records.map((record) => (
                                    <tr key={record.id} className="hover:bg-gray-50 transition">
                                        <td className="px-4 sm:px-6 py-3 sm:py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-semibold text-gray-900 text-sm sm:text-base">{record.restaurant_name}</p>
                                                    <p className="text-xs sm:text-sm text-gray-500">{record.city}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 sm:px-6 py-3 sm:py-4">
                                        ...
                                        <td className="px-4 sm:px-6 py-3 sm:py-4">
  <span className="inline-block px-2 py-1 text-[10px] sm:text-xs font-semibold rounded-full bg-indigo-100 text-indigo-700 whitespace-nowrap overflow-hidden text-ellipsis max-w-[80px] sm:max-w-none">
    {record.package || 'N/A'}
  </span>
</td>
                                        <td className="px-4 sm:px-6 py-3 sm:py-4">
                                            {record.application_status ? (
                                                <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-700">Accepted</span>
                                            ) : (
                                                <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-orange-100 text-orange-700">Pending</span>
                                            )}
                                        </td>
...
function DetailModal({ record, onClose, onAccept, processing }: DetailModalProps) {
    const [currentStep, setCurrentStep] = useState(1);
    ...
    const hasRestaurantTimings = record.timings?.hours && Object.keys(record.timings.hours).length > 0;
    const hasDeliveryTimings = record.delivery_timings?.hours && Object.keys(record.delivery_timings.hours).length > 0;
    const hasTakeawayTimings = record.takeaway_timings?.hours && Object.keys(record.takeaway_timings.hours).length > 0;

    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4">
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
                {/* Header */}
                <div className="bg-orange-100 px-4 sm:px-8 py-4 sm:py-6 ">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            <h2 className="text-xl sm:text-2xl font-bold">{record.restaurant_name}</h2>
                            <p className="text-orange-700 text-xs sm:text-sm mt-0.5">Application Review</p>
                        </div>
                        <button aria-label='close' onClick={onClose} className=" hover:bg-white/20 p-2 rounded-lg transition self-start sm:self-auto">
                ...
                {/* Step Navigation */}
                <div className="bg-gray-50 px-3 sm:px-6 py-3 sm:py-4 border-b border-orange-500 z-50">
                    <div className="flex justify-between max-w-3xl mx-auto min-w-max">
                        {steps.map((step, idx) => (
                            <div key={step.id} className="flex flex-col items-center min-w-[80px]">
                                <button
                ...
                <div className="p-4 sm:p-6 overflow-y-auto flex-1">
                    ...
                    {currentStep === 2 && (
                        <Section title="Business & Legal Details">
                            ...
                            <div className="mt-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {cuisines.map((c: string, i: number) => (
                                        <span key={i} className="px-2.5 py-1 bg-orange-100 text-orange-700 rounded-full text-xs sm:text-sm capitalize">
                                            {c}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div className="mt-3">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {services.map((s: string, i: number) => (
                                        <span key={i} className="px-2.5 py-1 bg-blue-100 text-blue-700 rounded-full text-xs sm:text-sm capitalize">
                                            {s.replace('_', ' ')}
                                        </span>
                                    ))}
                                </div>
                    ...
                    {currentStep === 3 && (
                        <Section title="Operating Hours">
                            <div className="space-y-4">
                                <div>
                                    <h4 className="font-semibold text-gray-900 mb-2 text-sm sm:text-base">Restaurant Timings</h4>
                                    {hasRestaurantTimings ? (
                                        <div className="space-y-2">
                                            {getTimingsArray(record.timings).map((day) => (
                                                <div key={day.day} className="flex justify-between items-center py-2 px-3 sm:px-4 bg-gray-50 rounded-lg">
                                                    <span className="font-medium text-gray-700 text-sm">{day.day}</span>
                                                    <div className="text-sm text-gray-600">
                                                        {day.slots.map((slot, i: number) => (
                                                            <span key={i} className="whitespace-nowrap">{slot.open_time} - {slot.close_time}</span>
                                                        ))}
                                                    </div>
                                                </div>
                                            ))}
                                        ...
                                        <p className="text-gray-500 italic text-sm">Timings not available</p>
                                    )}
                                </div>

                                {record.delivery_timings && (
                                    <div>
                                        <h4 className="font-semibold text-gray-900 mb-2 mt-4 text-sm sm:text-base">Delivery Timings</h4>
                                        {hasDeliveryTimings ? (
                                            <div className="space-y-2">
                                                {getTimingsArray(record.delivery_timings).map((day) => (
                                                    <div key={day.day} className="flex justify-between items-center py-2 px-3 sm:px-4 bg-blue-50 rounded-lg">
                                                        <span className="font-medium text-blue-700 text-sm">{day.day}</span>
                                                        <div className="text-sm text-blue-600">
                                                            {day.slots.map((slot, i: number) => (
                                                                <span key={i} className="whitespace-nowrap">{slot.open_time} - {slot.close_time}</span>
                                                            ))}
                                            ...
                                            <p className="text-gray-500 italic text-sm">Delivery timings not available</p>
                                        )}
                                    </div>
                                )}

                                {record.takeaway_timings && (
                                    <div>
                                        <h4 className="font-semibold text-gray-900 mb-2 mt-4 text-sm sm:text-base">Takeaway Timings</h4>
                                        {hasTakeawayTimings ? (
                                            <div className="space-y-2">
                                                {getTimingsArray(record.takeaway_timings).map((day) => (
                                                    <div key={day.day} className="flex justify-between items-center py-2 px-3 sm:px-4 bg-green-50 rounded-lg">
                                                        <span className="font-medium text-green-700 text-sm">{day.day}</span>
                                            ...
                                            <p className="text-gray-500 italic text-sm">Takeaway timings not available</p>
                                        )}
                                    </div>
                                )}
                            </div>
                        ...
                        <Section title="Documents & Images">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {record.logo_url && (
                                    <div>
                                        <p className="text-sm font-semibold text-gray-700 mb-2">Logo</p>
                                        <img src={`${STRAPI_URL}${record.logo_url.url}`} className="w-full h-32 sm:h-40 object-cover rounded-lg border border-gray-200" alt="Logo" />
                                    </div>
                                )}
                                {record.pan_card && (
                                    <div>
                                        <p className="text-sm font-semibold text-gray-700 mb-2">PAN Card</p>
                                        <a href={`${STRAPI_URL}${record.pan_card.url}`} target="_blank" rel="noopener noreferrer"
                                            className="block p-3 border-2 border-dashed border-gray-300 rounded-lg hover:border-orange-500 transition text-center">
                                            <svg className="w-8 h-8 sm:w-10 sm:h-10 mx-auto text-gray-400 mb-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                                            </svg>
                                            <p className="text-xs sm:text-sm text-gray-600 truncate px-1">{record.pan_card.name}</p>
                                        </a>
                                    </div>
                                )}
                                {record.fssai_license && (
                                    <div>
                                        <p className="text-sm font-semibold text-gray-700 mb-2">FSSAI License</p>
                                        <a href={`${STRAPI_URL}${record.fssai_license.url}`} target="_blank" rel="noopener noreferrer"
                                            className="block p-3 border-2 border-dashed border-gray-300 rounded-lg hover:border-green-500 transition text-center">
                                            <svg className="w-8 h-8 sm:w-10 sm:h-10 mx-auto text-gray-400 mb-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                            </svg>
                                            <p className="text-xs sm:text-sm text-gray-600 truncate px-1">{record.fssai_license.name}</p>
                                        </a>
                                    </div>
                                )}
                                {record.gst_certificate && (
                                    <div>
                                        <p className="text-sm font-semibold text-gray-700 mb-2">GST Certificate</p>
                                        <a href={`${STRAPI_URL}${record.gst_certificate.url}`} target="_blank" rel="noopener noreferrer"
                                            className="block p-3 border-2 border-dashed border-gray-300 rounded-lg hover:border-blue-500 transition text-center">
                                            <svg className="w-8 h-8 sm:w-10 sm:h-10 mx-auto text-gray-400 mb-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                                            </svg>
                                            <p className="text-xs sm:text-sm text-gray-600 truncate px-1">{record.gst_certificate.name}</p>
                                        </a>
                                    </div>
                                )}
                            </div>
                ...
                {/* Footer Actions */}
                <div className="border-t border-gray-300 bg-gray-50 px-4 sm:px-6 py-4">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div className="flex gap-2">
                            <button
                                onClick={() => setCurrentStep(s => Math.max(1, s - 1))}
                                disabled={currentStep === 1}
                                className="px-3 py-1.5 sm:px-4 sm:py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition font-medium text-xs sm:text-sm"
                            >
                                ← Previous
                            </button>
                            <button
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={onClose}
                                className="px-3 py-1.5 sm:px-4 sm:py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 transition font-medium text-xs sm:text-sm"
                            >
                                Close
                            </button>
                            {!record.application_status && (
                                <>
                                    <button
                                        onClick={() => alert('Reject functionality')}
                                        disabled={processing}
                                        className="px-3 py-1.5 sm:px-4 sm:py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 disabled:opacity-50 transition font-medium text-xs sm:text-sm"
                                    >
                                        Reject
                                    </button>
                                    <button
                                        onClick={() => onAccept(record)}
                                        disabled={processing}
                                        className="px-3 py-1.5 sm:px-4 sm:py-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:shadow-lg disabled:opacity-50 transition font-medium text-xs sm:text-sm"
                                    >
                                        {processing ? 'Processing...' : '✓ Accept'}
                                    </button>
                                </>
                            ...
                            {record.application_status && (
                                <div className="px-3 py-1.5 sm:px-4 sm:py-2 bg-green-100 text-green-700 rounded-lg font-semibold text-xs sm:text-sm">
                                    ✓ Accepted
                                </div>
                            )}
                        </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
    return (
        <div>
            <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3 pb-1.5 border-b border-gray-300">{title}</h3>
            {children}
        </div>
    );
}
